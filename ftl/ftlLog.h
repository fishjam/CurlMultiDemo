...
namespace FTL
{
#define FTL_LOG_VERSION     60          //0.60

//以下部分用于 FTL_LOG_SHARE_META_INFO::dwFlags 部分

//允许对 ftlLog 进行调试(本身是输出日志的, 无法直接使用 API_VERIFY/FLTRACE 等)
#define FTL_LOG_META_FLAG_DEBUG              (0x0001)
...
//#define FTL_LOG_SHARE_SIZE          (4096)   //4K
    ...
    struct FTL_LOG_SHARE_META_INFO {
        DWORD dwVersion;                //版本(避免多个版本的冲突?)
        DWORD dwFlags;                  //标志位, FTL_LOG_FLAG_DEBUG 等
        ...
        ULONG volatile nFileIndex;       //0 开始的文件序号.
...
#define LOG_MSG_BUFFER_SIZE         (FTL_LOG_SHARE_SIZE - LOG_MSG_INFO_SIZE)
    ...
    public:
        ...
        virtual BOOL WriteLogBuffer(LPCVOID pBuffer, LONG nBufLen, LONG* pWritten) = 0;
    ...
    public:
        ...
        FTLINLINE virtual BOOL WriteLogBuffer(LPCVOID pBuffer, LONG nBufLen, LONG* pWritten);
    ...
    #define LOGGER_TRACE_OPTION_FLAGS                       0x0001
    #define LOGGER_TRACE_OPTION_LOG_PATH                    0x0002
    #define LOGGER_TRACE_OPTION_LOG_THRESHOLD               0x0004
    #define LOGGER_TRACE_OPTION_LOG_TARGET                  0x0008
    #define LOGGER_TRACE_OPTION_DATETIME_FMT                0x0010
    #define LOGGER_TRACE_OPTION_MAX_LOG_FILE_SIZE           0x0020
    #define LOGGER_TRACE_OPTION_MAX_TOTAL_LOG_SIZE          0x0040

    #define LOGGER_TRACE_OPTION_ALL                         0xFFFF

    struct LoggerTraceOptions
    {
        DWORD dwOptionsFlags;   //refer LOGGER_TRACE_OPTION_xxx , 表明本结构体中哪些元素有效

        DWORD dwMetaFlags;      //refer FTL_LOG_META_FLAG_DEBUG 等
        LPCTSTR pszLogPath;
        TraceLevel logThreshold;
        DWORD logTarget;
        LogDateTimeFormat dateTimeFmt;      //控制日期时间格式
    ...
    public:
        ...
        //以 SYSTEMTIME 的格式返回当前的本地时间, 其 wMilliseconds 设置成 0
        //  注意: 不能通过 wMilliseconds 的值返回微秒数(其为 WORD, 最多只能存 65535)
        FTLINLINE SYSTEMTIME GetLocalTimeAsSystemTimeWithMicroSec(BOOL highPrecision, DWORD& dwOutMicroSec);
    ...
    public:
        ...
        FTLINLINE BOOL GetTraceOptions(LoggerTraceOptions& traceOptions);
        ...
        FTLINLINE BOOL /*__cdecl*/ WriteLogInfoExW(TraceLevel level, const LPCWSTR lpszFormat, ...);
        FTLINLINE BOOL /*__cdecl*/ WriteLogInfoExA(TraceLevel level, const LPCSTR lpszFormat, ...);
    ...
    protected:
        ...
        FTLINLINE BOOL _CheckAndUpdateByMetaFlags(DWORD dwMetaFlags);
...
#   define FTL_TRACE_EX_W_FULL(_level, _file, _line, _fun, fmt, ...) \
    do{ \
        ...
        if (logger.CheckLevel(_level)){ \
            ...
            logger.WriteLogInfoExW(_level, L"%s(%d):%u|%s|%04d|%04d|%s|%s|" fmt LOG_LINE_SEP_W, \
                ...
                GetCurrentProcessId(), GetCurrentThreadId(), logger.GetLevelName(_level), _fun, __VA_ARGS__); \
        } \
    }while(0);

#   define FTL_TRACE_EX_A_FULL(_level, _file, _line, _fun, fmt, ...) \
    do{ \
        FTL::CFLogger& logger = FTL::CFLogger::GetInstance(); \
        if (logger.CheckLevel(_level)){ \
            CHAR szTimeStr[32] = {0}; \
...
#   define FTLTRACEEXW(_level, fmt, ...) FTL_TRACE_EX_W_FULL(_level, FTL::CFLogger::GetBaseNameW(TEXT(__FILE__)), __LINE__, __FUNCTIONW__, fmt, __VA_ARGS__)
#   define FTLTRACEEXA(_level, fmt, ...) FTL_TRACE_EX_A_FULL(_level, FTL::CFLogger::GetBaseNameA(__FILE__), __LINE__, __FUNCTION__, fmt, __VA_ARGS__)
